"""Live Amme API MCP server.

Exposes the reverse-engineered Amme personal-finance API (https://api.emma-app.com)
as MCP tools over stdio. Reads are always available; write tools edit Amme
...
_NO_BANK_MOVEMENT = (
    "This tool edits Amme metadata only (names/categories/labels/manual entries). "
...
mcp = FastMCP(
    name="amme",
    instructions=(
        "Amme personal finance assistant over the live API (api.emma-app.com). "
        "Read tools are always available. Write tools edit Amme metadata only — they "
...
def _wrap(endpoint: str, **payload) -> dict:
    ...
    return {"source": "amme-api", "endpoint": endpoint, "fetched_at": _now(), **payload}
...
def get_balances(include_hidden: bool = False) -> dict:
    """All linked Amme accounts with balances (from /bank-connections).
...
def get_account(account_id: int | None = None, name: str | None = None) -> dict:
    """Fetch one Amme account by id, or by case-insensitive name match.
...
def update_manual_account(
    ...
    — it only updates Amme's manual-account metadata."""
    account = api.sanitize_account_detail(api.get(f"/accounts/{account_id}"))
    if account.get("provider") != "MANUAL":
        return {
            "error": f"Account {account_id} is not manual (provider={account.get('provider')!r}). "
